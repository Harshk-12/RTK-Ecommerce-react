...
// import { setcounter } from "./redux/Slices/commonSlice";
// import { UseSelector, useSelector } from "react-redux/es/hooks/useSelector";
import { setcounter,setcart,setsubtotal,settotalamout } from "./redux/Slices/commonSlice";

function Cart() {
let dispatch=useDispatch()
  let {counter,cartindex,totalamount,subtotal}=useSelector((state)=>state.common)
  //
  ...
  let remove = (e, index) => {
    ...
    dispatch(setcart(filteredarray));
  ...
  useEffect(() => {
    ...
    dispatch(settotalamout(Math.floor(total)))
  }, [cartindex]);
      ...
      <section className={counter === 0 ? "hideaccount" : " "}>
        ...
        <div className='row gx-5'>
            ...
            <div className='mt-5'>
              <div className='px-5'>
                {cartindex.map((item, index) => {
                  return (
                    <>
                      <div className=' mt-5 d-flex align-items-center ' style={{ gap: "8vw" }} key={index}>
                        {/* product details */}
                        <div className='cartitems d-flex align-items-center '>
                          ...
                          <div className='ms-4' style={{ width: "20vw" }}>
                            <p>{item.title}</p>
                            <p className='text-danger' style={{ cursor: "pointer" }} onClick={e => remove(e, index)}>
                              Remove
                            </p>
                            {/* {console.log(index)} */}
                          </div>
                        ...
                        <form className=' cart-quantity '>
                          <input
                            ...
                            defaultValue='1'
                            value={item.quantity}
                            onChange={e => quantityChange(parseInt(e.target.value), index)}
                          />
                        </form>
                        ...
                        <div className=' fs-5 fw-normal'>
                          <p style={{ width: "90px" }}>${item.quantity * item.price}</p>
                        </div>
                      </div>
                    </>
                  );
          ...
          <div className='col-3'>
            ...
            <div className='mt-3'>
              <h5>Promo Code</h5>
              <form action='/' className='d-flex flex-column border-bottom pb-5'>
                <input type='text' className='form-control' placeholder='Enter the Promo Code' />
