...
function Navbar() {
  ...
  let loginuseremail = sessionStorage.getItem("Logindetail") ? JSON.parse(sessionStorage.getItem("Logindetail"))?.username : null;
  console.log(loginuseremail, "lg");
  let emailindex = loginuseremail?.indexOf("@");

  let loginusername = logindata ? loginuseremail?.slice(0, emailindex) : "sigin";
  ...
  let searchitem = value => {
    ...
    dispatch(
      setfilterdata(
        data2.filter(item => {
          return searchvalue ? item.title.toUpperCase().includes(searchvalue) : "";
        })
      )
    );
    ...
    <>
      <nav className='navbar navbar-expand-lg  customnavbar d-flex align-items-center'>
        <div className='container-fluid '>
          <div
            className=' navlist me-5'
            style={{ fontSize: "1.4em", cursor: "pointer", height: "60px", width: "200px" }}
            onClick={() => {
              navigate("/");
            }}>
          ...
          <div className=' d-flex align-items-center justify-content-center mb-3'>
            ...
            <button className='mt-4 pt-1 rounded-end px-3 ms-1 pb-2 btn btn-secondary'>
              <AiOutlineSearch />{" "}
            </button>
          </div>
          {/*  */}
                ...
                return (
                  <>
                    <li
                      key={item.id}
                      onClick={() => {
                        return searchFunc(item.title, item.id);
                      }}
          ...
          <div className='collapse navbar-collapse d-flex ms-5 navlistitems' id='navbarNav'>
              ...
              {navbarList.map((item, index) => {
                return (
                  <li className='nav-item ' key={index}>
                    <Link className='nav-link text-dark navlist ' aria-current='page' to={item.to}>
                      {item.label}
                    </Link>
                  </li>
                );
            ...
            <div className='ms-auto'>
              <div className='counter ' style={{ color: "#f55742" }}>
                {counter}
              </div>
              <ShoppingCartIcon
